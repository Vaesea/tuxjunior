//  $Id: addon.cpp 4966 2007-04-04 20:01:53Z sommer $
//
//  SuperTux - Add-on
...
Addon::parse(std::string fname)
{
  try {
    ...
    const lisp::Lisp* addon = root->get_lisp("tuxjunior-addoninfo");
    if(!addon) throw std::runtime_error("file is not a tuxjunior-addoninfo file.");
    parse(*addon);
  } catch(std::exception& e) {
    std::stringstream msg;
...
{
  writer.start_list("tuxjunior-addoninfo");
  ...
  writer.end_list("tuxjunior-addoninfo");
}
