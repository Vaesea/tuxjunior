//  $Id: worldmap.cpp 5484 2008-05-16 15:42:02Z sik0fewl $
//
//  tuxjunior -  A Jump'n Run
...
WorldMap::load(const std::string& filename)
{
  ...
  try {
    ...
    if(level == NULL)
      throw std::runtime_error("file isn't a tuxjunior-level file.");
...
WorldMap::get_level_title(LevelTile& level)
{
  ...
  try {
    ...
    const lisp::Lisp* level_lisp = root->get_lisp("tuxjunior-level");
    if(!level_lisp)
      return;
