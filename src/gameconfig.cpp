//  $Id: gameconfig.cpp 5147 2007-08-20 17:15:04Z tuxdev $
//
...
Config::load()
{
  ...
  const lisp::Lisp* config_lisp = root->get_lisp("tuxjunior-config");
  if(!config_lisp)
    throw std::runtime_error("File is not a tuxjunior-config file");
...
Config::save()
{
  ...
  writer.start_list("tuxjunior-config");
  ...
  writer.end_list("tuxjunior-config");
}
