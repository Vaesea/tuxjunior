//  $Id: addon_manager.cpp 5228 2007-12-20 23:11:01Z sommer $
//
...
AddonManager::get_available_addons() const
{
  ...
  const char* baseUrl = "http://gluebox.com/addons/index.nfo";
  std::string addoninfos = "";

  CURL *curl_handle;
  curl_handle = curl_easy_init();
  curl_easy_setopt(curl_handle, CURLOPT_URL, baseUrl);
  curl_easy_setopt(curl_handle, CURLOPT_USERAGENT, "TuxJunior/" PACKAGE_VERSION " libcURL");
  ...
  try {
    ...
    const lisp::Lisp* root = parser.parse(addoninfos_stream, "tuxjunior-addons");

    const lisp::Lisp* addons_lisp = root->get_lisp("tuxjunior-addons");
    if(!addons_lisp) throw std::runtime_error("Downloaded file is not an Add-on list");

    lisp::ListIterator iter(addons_lisp);
    while(iter.next()) {
      const std::string& token = iter.item();
      if(token == "tuxjunior-addoninfo") {
        Addon addon;
