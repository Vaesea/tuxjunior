//  $Id: player.cpp 5256 2008-01-06 02:46:38Z sommer $
//
//  SuperTux
    ...
    grabbed_object(NULL), ghost_mode(false), edit_mode(false), climbing(0)
{
  ...
  sound_manager->preload("sounds/freesound/JUMP_7737_NoiseCollector_pigtoy.wav");
  sound_manager->preload("sounds/hurt.ogg");
  sound_manager->preload("sounds/freesound/SLIDE_19277_martian_super_cold_slurp.wav");
...
Player::handle_horizontal_input()
{
  ...
  if(on_ground() && ((vx < 0 && dirsign >0) || (vx>0 && dirsign<0))) {
    ...
    if(fabs(vx)>SKID_XM && !skidding_timer.started()) {
      ...
      sound_manager->play("sounds/freesound/SLIDE_19277_martian_super_cold_slurp.wav");
      // dust some particles
